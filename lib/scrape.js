 ...
 * @property {string} url
 * @property {string|import('./querySelectorAll.js').SelectorFunction} selector
 * @property {RegExp} pattern
 * @property {boolean} [javaScriptEnabled]
 */

/**
 * @typedef {Object} LoadErrorContext
 * @property {Target} target
 */

/**
 * @callback OnLoadError
 * @param {LoadErrorContext} context
 ...
 * @property {OnLoadError} [onLoadError]
...
export default async function scrape(pTarget, pOptions = {}) {
	...
	const { onLoadError, onNoMatch, onMatch } = pOptions
	...
	try {
		...
		if (onLoadError) {
			await onLoadError({ target: pTarget })
		}
