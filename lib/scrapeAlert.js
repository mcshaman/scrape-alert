...
export default async function scrapeAlert(pTargets, pOptions) {
	...
	const scrapeOptions = {
		async onLoadError(pContext) {
			const { target } = pContext
			const { name, url } = target

			try {
				await results.add({
					target,
					message: `The url ${url} failed to load`,
					error: true,
				})
			} catch (pError) {
		...
		async onNoMatch(pContext) {
			const { target, screenshot } = pContext
			const { name, selector, url } = target

			try {
				await results.add({
					target,
					message: `No matches for the CSS selector '${selector}' were found at ${url}. This could mean that the structure of the page has changed and the CSS selector may need to be modified.`,
				})
			} catch (pError) {
				log(`Failed to record result - ${pError}`, { prefix: name })
			}
		...
		async onMatch(pContext) {
			const { target, screenshot, matchCount } = pContext
			const { name, url } = target

			try {
				await results.add({
					target,
					message: `${matchCount} matches found at ${url}`,
					matchCount,
				})
			} catch (pError) {
				log(`Failed to record result - ${pError}`, { prefix: name })
