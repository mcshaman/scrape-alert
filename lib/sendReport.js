...
function makeMessage(pResult) {
	...
	if (error) {
		return `The url ${target.url} for scrape target ${target.name} failed to load`
	}

	if (matchCount) {
		return `${matchCount} matches found at ${target.url} for scrape target ${target.name}`
	}

	// No matches
	return `No matches for the CSS selector '${target.selector}' were found at ${target.url} for scrape target ${target.name}. This could mean that the structure of the page has changed and the CSS selector may need to be modified.`
}
